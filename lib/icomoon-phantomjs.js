...
function waitFor(checkFn, times, cb) {
  var i = 0;
  function runWaitFor() {
    if (checkFn()) {
      cb();
      ...
      cb(new Error('Timeout of ' + (times * 100) + ' reached. This usually means something went wrong. Please try your SVGs inside icomoon itself, http://icomoon.io/app-old'));
    }
  }
  runWaitFor();
}
